...
class User(AbstractUser):
    ...
    first_major = models.CharField(max_length=20)                 # 본전공
    second_major = models.CharField(max_length=20, null=True, blank=True) # 2전공
    third_major = models.CharField(max_length=20, null=True, blank=True)  # 3전공
...
class Crew(models.Model):
    ...
    category = models.ForeignKey(Category, related_name="crew", on_delete=models.CASCADE)
...
class Post(models.Model):
    ...
    final_result_date = models.DateTimeField()
    requirement_target = models.CharField(max_length=300)  # 모집 대상 명시 텍스트
    title = models.CharField(max_length=100)   # 공고 제목
    content = models.TextField()  # 공고 내용
    membership_fee = models.CharField(max_length=100)  # 회비 내용
...
class PostImage(models.Model):
    ...
    post = models.ForeignKey(related_name='image', on_delete=models.CASCADE)
...
class Apply(models.Model):
    ...
    document_pass = models.BooleanField(default=False) # 서류 통과 여부, 기본값 False
    interview_time_date = models.DateTimeField(null=True, blank=True) # 면접 날짜 정한 것
    final_pass = models.BooleanField(default=False)    # 최종합격 여부, 기본값 False
...
class Section(models.Model):
    ...
    document = models.ForeignKey(Post, related_name='section', on_delete=models.CASCADE)
...
class LongSentence(models.Model):
    ...
    answer_limit = models.IntegerField()
    is_essential = models.BooleanField(default=False) # 해당 타입이 필수적인지 아닌지 여부
    section = models.ForeignKey(Section, related_name="longsentence", on_delete=models.CASCADE)
...
class CheckBox(models.Model):
    question = models.CharField(max_length=100) 
    is_essential = models.BooleanField(default=False) # 해당 타입이 필수적인지 아닌지 여부
    count_minumum = models.IntegerField()
    count_maximum = models.IntegerField()
    section = models.ForeignKey(Section, related_name="checkbox", on_delete=models.CASCADE)
